    ...
    public int run(CommandContext<CommandSource> context) throws CommandSyntaxException {
        ...
        if (player != null) {
            ...
            if (ForgeEventHandlers.debug) {
                player.sendMessage(new StringTextComponent("Enabled FxControl debug mode"), Util.NIL_UUID);
            } else {
                player.sendMessage(new StringTextComponent("Disabled FxControl debug mode"), Util.NIL_UUID);
            }
        }
        return 0;
    }
